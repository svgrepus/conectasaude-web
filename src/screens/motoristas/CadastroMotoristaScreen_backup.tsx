...
const CadastroMotoristaScreen: React.FC<CadastroMotoristaScreenProps> = ({
  ...
  const preencherFormularioEdicao = (motorista: MotoristaCompleto) => {
  ...
  const handleSave = async () => {
    ...
    try {
      const motoristaData: CriarMotoristaData = {
        ...
        escalas: [] // Vazio por enquanto para simplificar
      };

      if (isEdit && motoristaToEdit?.id) {
        await motoristasService.atualizarMotorista(motoristaToEdit.id, motoristaData);
        setMessage({ type: 'success', text: 'Motorista atualizado com sucesso!' });
      } else {
        await motoristasService.criarMotorista(motoristaData);
