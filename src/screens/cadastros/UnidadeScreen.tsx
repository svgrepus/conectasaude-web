...
export const UnidadeScreen: React.FC = () => {
  ...
  const loadUnidades = async (page: number = 1, search?: string) => {
    try {
      setLoading(true);
      console.log('🔍 UnidadeScreen: Carregando unidades', { page, search, searchQuery });

      const response = await unidadeService.getUnidades(page, itemsPerPage, search);
      
      console.log('📦 UnidadeScreen: Resposta recebida:', response);
      
      if (response.data) {
        setUnidades(response.data);
        setCurrentPage(page);
        setTotalCount(response.count || 0);
        console.log('✅ UnidadeScreen: Estado atualizado', {
          unidadesCount: response.data.length,
  ...
  useEffect(() => {
    console.log('🚀 UnidadeScreen: Componente montado, carregando dados iniciais');
    loadUnidades(1);
  }, []);
  ...
  const handleSave = async (data: { nome: string; codigo?: string; endereco?: string; telefone?: string; email?: string; codigo_cnes?: string; tipo_unidade: 'UBS' | 'ESF' | 'HOSPITAL' | 'CLINICA' | 'CENTRO_ESPECIALIDADES' | 'OUTROS'; ativa?: boolean }) => {
    try {
      setLoading(true);
      
      if (selectedUnidade) {
        // Editando
        await unidadeService.updateUnidade(selectedUnidade.id, data);
      } else {
        // Criando
      ...
      // Recarregar a lista
      await loadUnidades(currentPage, searchQuery);
      
      // Fechar o formulário
      setFormVisible(false);
      setSelectedUnidade(null);
      
      Alert.alert('Sucesso', selectedUnidade ? 'Item atualizado com sucesso!' : 'Item criado com sucesso!');
    } catch (error) {
  ...
  const handleDelete = async (unidade: Unidade) => {
    console.log('🚨 HANDLE DELETE CHAMADO!', unidade);
    console.log('🚨 ID:', unidade.id, 'Nome:', unidade.nome);
  ...
  const confirmDelete = async () => {
    if (!unidadeToDelete) {
      console.log('❌ unidadeToDelete é null, cancelando exclusão');
      return;
    }
    ...
    try {
      ...
      await unidadeService.deleteUnidade(unidadeToDelete.id);
  ...
  const handlePageChange = (page: number) => {
    if (page !== currentPage && page >= 1 && page <= totalPages && !loading) {
      loadUnidades(page, searchQuery);
    }
  };

  // Função para obter o label do tipo de unidade
  const getTipoUnidadeLabel = (tipo: string) => {
    ...
    <View style={[styles.tableRow, { borderTopColor: currentTheme.border }]}>
      ...
      <View style={styles.typeCell}>
        <Text style={[styles.cellTextSecondary, { color: currentTheme.mutedForeground }]}>
          {getTipoUnidadeLabel(item.tipo_unidade || 'UBS')}
        </Text>
      </View>
            ...
            {unidades.length === 0 ? (
              <View style={styles.emptyContainer}>
                <Text style={[styles.emptyText, { color: currentTheme.mutedForeground }]}>
                  {searchQuery ? 'Nenhum resultado encontrado para a busca.' : 'Nenhuma unidade cadastrada ainda.'}
                </Text>
              </View>
            ) : (
              <FlatList
        ...
        <View style={styles.modalOverlay}>
          <View style={styles.modalContainer}>
            ...
            <Text style={styles.modalMessage}>
              Deseja realmente deletar a unidade "{unidadeToDelete?.nome}"?
            </Text>
